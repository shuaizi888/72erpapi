...
class TestBGameScenario(unittest.TestCase):#点72后台接口测试场景
    ...
    def testbiaoserverhulogin(self, data):
        ...
        data['standardPrepareFindgoodsdata']['params']['sessionUuid'] = machineCode
        res = scripts.loadtestInterface(
            instance=HttpWebRequest(),
            instance_pro='post',
            data=json.dumps(data['standardPrepareFindgoodsdata']),
            desc=data['Desc'],
            url=data['GameUrl']
        )
        #断言
        price = res['data'][1]['price']
        specRemark = res['data'][1]['specRemark']

        print price
        print specRemark
        self.assertEqual(res['code'],0,res['msg'])#断言
        self.assertTrue(res['data'][2]['price'])
        self.assertTrue(res['data'][2]['specRemark'])
...
if __name__=="__main__":
    ...
    filePath = os.path.join(gl.reportPath, 'gameReporthulogin.html')  # 确定生成报告的路径
    # print filePath
